...
describe('ui', () => {
  ...
  describe('renderChannelList', () => {
    const channels = [
      { id: 'UC1', handle: '@chan1', title: 'Channel 1', thumbnailUrl: 'https://img/1.jpg' },
      { id: 'UC2', handle: '@chan2', title: 'Channel 2', thumbnailUrl: 'https://img/2.jpg' },
    ];
  ...
  describe('renderStreams', () => {
    const streams = [
      {
        videoId: 'v1', title: 'Live Stream', channelTitle: 'Ch1',
        thumbnailUrl: 'https://img/t1.jpg', eventType: 'live',
        concurrentViewers: '2500'
      },
      {
        videoId: 'v2', title: 'Upcoming Stream', channelTitle: 'Ch2',
        thumbnailUrl: 'https://img/t2.jpg', eventType: 'upcoming',
        scheduledStartTime: new Date(Date.now() + 30 * 60000).toISOString()
      },
      {
        videoId: 'v3', title: 'Recent Stream', channelTitle: 'Ch3',
        thumbnailUrl: 'https://img/t3.jpg', eventType: 'completed',
    ...
    it('calls onClick with videoId when card is clicked', () => {
      let clicked = null;
      ui.renderStreams('live-streams', [streams[0]], (id) => { clicked = id; });
      const container = dom.elements.get('live-streams');
    ...
    it('creates live badge for live streams', () => {
      ...
      const badges = thumbWrapper.children.filter(c => c.className === 'live-badge');
    ...
    it('creates scheduled badge for upcoming streams', () => {
      ...
      const card = container.children[0];
      const thumbWrapper = card.children[0];
      const badges = thumbWrapper.children.filter(c => c.className === 'scheduled-badge');
      assert.equal(badges.length, 1);
    });
  });
