...
describe("App Integration", () => {
  ...
  describe("Channel selection controls stream fetching", () => {
    it("only fetches live streams on init, not upcoming or completed", async () => {
      ...
      const searchCalls = fetchCalls.filter(
        (u) => u.includes("/search?") && u.includes("type=video"),
      );
    ...
    it("fetches upcoming and completed only for selected channel", async () => {
      ...
      for (const call of searchCalls) {
        assert.ok(
          call.includes("UC_CREATOR"),
          "Should only fetch for selected channel",
        );
        assert.ok(
          !call.includes("UC_SECOND"),
          "Should not fetch for unselected channel",
        );
      }
    });
  });
  ...
  describe("UI rendering integration", () => {
    it("renders channel list with selection state", async () => {
      ...
      list.innerHTML = "";
      list.children = [];
      ui.renderChannelList(channels, "UC2", {
        onSelect: () => {},
        onRemove: () => {},
      });
    ...
    it("renders live stream cards with badges and viewer count", async () => {
      ...
      const liveBadge = thumbWrapper.children.find(
        (c) => c.className === "live-badge",
      );
      const viewerCount = thumbWrapper.children.find(
        (c) => c.className === "viewer-count",
      );
      assert.ok(liveBadge);
  ...
  describe("Full workflow: add channel -> fetch live -> select -> fetch upcoming/completed", () => {
    it("completes the entire user workflow", async () => {
      ...
      assert.ok(
        dom.elements.get("upcoming-section").classList._classes.has("hidden"),
      );
      ...
      assert.ok(
        !dom.elements.get("completed-section").classList._classes.has("hidden"),
      );
      ...
      const upcomingCalls = fetchCalls.filter((u) =>
        u.includes("eventType=upcoming"),
      );
      const completedCalls = fetchCalls.filter((u) =>
        u.includes("eventType=completed"),
      );
