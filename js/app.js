...
function wireEvents() {
  ...
  document.getElementById('close-player').addEventListener('click', () => ui.hidePlayer());
...
async function refreshLive() {
  ...
  try {
    ...
    allStreams.live = allStreams.live.map(s => ({ ...s, ...(detailMap.get(s.videoId) || {}) }));
  } catch { /* non-critical */ }
...
async function refreshSelectedChannel() {
  ...
  try {
    ...
    const detailMap = new Map(details.map(d => [d.videoId, d]));
    const enrich = (streams) => streams.map(s => ({ ...s, ...(detailMap.get(s.videoId) || {}) }));
...
function startAutoRefresh() {
  ...
  refreshTimer = setInterval(() => {
    if (!youtube.isQuotaSafe()) {
      stopAutoRefresh();
      ui.showToast('Auto-refresh paused: approaching daily quota limit', 'warning');
      return;
    }
    ...
    youtube.fetchLiveStreams(channel.id)
