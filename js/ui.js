...
export function renderChannelList(channels, selectedId, { onSelect, onRemove }) {
  ...
  for (const channel of channels) {
    ...
    const item = el('li', { className: `channel-item${isActive ? ' active' : ''}` }, [
      el('img', {
      ...
      el('div', { className: 'channel-info' }, [
        ...
        el('div', { className: 'channel-handle', textContent: channel.handle })
      ]),
      el('button', {
...
function createStreamCard(stream, onClick) {
  ...
  if (stream.eventType === 'live') {
    ...
    if (stream.concurrentViewers) {
      overlays.push(el('span', { className: 'viewer-count', textContent: formatViewers(stream.concurrentViewers) }));
    }
  } else if (stream.eventType === 'upcoming' && stream.scheduledStartTime) {
    overlays.push(el('span', { className: 'scheduled-badge', textContent: formatTime(stream.scheduledStartTime) }));
  }

  const card = el('div', { className: 'stream-card', 'data-video-id': stream.videoId }, [
    el('div', { className: 'thumbnail-wrapper' }, [
      el('img', { src: stream.thumbnailUrl, alt: stream.title, loading: 'lazy' }),
      ...overlays
    ]),
    el('div', { className: 'stream-info' }, [
      el('div', { className: 'stream-title', textContent: stream.title }),
      el('div', { className: 'stream-meta', textContent: stream.channelTitle + (stream.eventType === 'completed' ? ' \u2022 ' + timeAgo(stream.actualStartTime || stream.publishedAt) : '') })
    ])
  ]);
...
export function renderStreams(sectionId, streams, onStreamClick) {
  ...
  if (!streams.length) {
    container.appendChild(el('div', { className: 'stream-empty', textContent: 'No streams found.' }));
    return;
  }
...
export function renderEmptyState(sectionId, message) {
  const container = clearEl(sectionId);
  if (!container) return;
  container.appendChild(el('div', { className: 'stream-empty', textContent: message }));
}
